document.addEventListener('DOMContentLoaded', function() {
    ...
    var calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        eventClick: function(info) {
            ...
            // Add "Mark as Completed" button for booked/confirmed/in_progress events
            if (extendedProps.status && ['booked', 'confirmed', 'in_progress'].includes(extendedProps.status)) {
                detailsHTML += `
                    <button class="btn btn-success complete-event-btn" data-event-id="${eventId}">
    ...
    document.addEventListener('click', function(event) {
        if (event.target.classList.contains('complete-event-btn') || event.target.closest('.complete-event-btn')) {
            const button = event.target.classList.contains('complete-event-btn') ? event.target : event.target.closest('.complete-event-btn');
            const eventId = button.dataset.eventId;
            
            if (confirm('Are you sure you want to mark this event as completed? This will generate an invoice.')) {
                // Disable button and show loading state
                button.disabled = true;
                button.innerHTML = '<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Completing...';
                
                // Make AJAX request to complete the event
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        button.innerHTML = '<i class="fas fa-check-circle me-1"></i>Mark as Completed';
                    }
                })
                .catch(error => {
